...
import {CalledError, DisconnectedError, NotConnectedError,
  TimeoutError, ResponseFormatError, ConnectionError,
  CalledNotInitializedError} from './errors'
    ...
    return this.request(request).then((data: any) => {
      if (_.isEmpty(data) || !data.ledger_index) {
        return this._disconnect(false).then(() => {
          throw new CalledNotInitializedError('Called not initialized')
        })
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(eventName, response => {
        if (response.status === 'error') {
          _reject(new CalledError(response.error))
