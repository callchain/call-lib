...
function createEscrowCreationTransaction(account: string,
  ...
  const txJSON: any = {
    ...
    Amount: callToDrops(payment.amount)
  }
  ...
  if (payment.allowCancelAfter !== undefined) {
    txJSON.CancelAfter = iso8601ToCallTime(payment.allowCancelAfter)
  }
  if (payment.allowExecuteAfter !== undefined) {
    txJSON.FinishAfter = iso8601ToCallTime(payment.allowExecuteAfter)
  }
